...
- Add typed, read-only PCB manufacturing exports `pcb.export.gerber`, `pcb.export.pick_and_place` and `pcb.export.model3d`, wrapping the `@beta` `eda.pcb_ManufactureData.getGerberFile` / `getPickAndPlaceFile` / `get3DFile` and replacing the `debug exec` workaround. Each validates its arguments against the documented signature (Gerber accepts mm/inch only, pick-and-place mm/mil only), refuses a provably non-PCB active document with zero writes, reports an `undefined` platform result as a typed error naming the API, and returns the `File` on the existing artifact path. An unreadable document-identity probe fails open, because the host can render a live PCB while DMT metadata reads are empty.
- Add `easyeda pcb export-gerber`, `easyeda pcb export-pnp` and `easyeda pcb export-3d` with self-describing help, `--out` and `--json`. After saving, `export-gerber` lists the archive entries with pure-Go `archive/zip`, classifies them as copper/silkscreen/soldermask/paste/outline/drill/report, and warns when copper layers, drill files or the board outline are missing, so an Agent can check layers and drills without a CAM viewer.
- Validation boundary for these exports: connector unit tests and Go helper tests are offline only. The documented archive contents, and the observation that the pick-and-place `csv` export is UTF-16 tab-separated, come from EasyEDA Pro desktop 3.2.149 on Windows 11 through raw `debug exec` calls to the same APIs — not from the new typed path. No EasyEDA window was connected while this work was written, the Go code was not compiled because this host has no Go toolchain, and `get3DFile` has never been exercised live at all.
